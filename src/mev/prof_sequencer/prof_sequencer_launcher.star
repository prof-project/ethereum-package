    ...
    plan.exec(
        ...
        recipe=ExecRecipe(
            command=[
                "/bin/sh",
                "-c",
                "./run init -r {0} -k {1} -u {2} -s sequencer.json".format(
                    el_uri,
...
def spam_in_background(
    ...
    command = [
        "/bin/sh",
        "-c",
        "nohup ./run spam -r {0} -k {1} -u {2} -l sequencer.json  --secondsPerBundle {3} >sequencer.log 2>&1 &".format(
    ...
    if mev_flood_extra_args:
        ...
        command = [
            "/bin/sh",
            "-c",
            "nohup ./run spam -r {0} -k {1} -u {2} -l sequencer.json  --secondsPerBundle {3} {4} >sequencer.log 2>&1 &".format(
                el_uri, owner, user, seconds_per_bundle, joined_extra_args
            ),
        ]
    plan.exec(service_name="prof-sequencer", recipe=ExecRecipe(command=command))
